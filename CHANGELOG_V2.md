...
ClarificationAgent change, not a NudgeAgent one, and remains fully
unbuilt (see below).
...
implemented. Listed here so the reasoning isn't lost, and so none of it
gets accidentally described as a real feature before it exists.

### 3b. Task duration/pacing reasoning (ClarificationAgent) -- remaining, unbuilt half of item 3

- Few-shot examples teaching ClarificationAgent to distinguish (a)
  no-fixed-end habits -- no duration question at all, needs a
  progression-curve mechanism instead -- from (b) fixed-external-deadline
tasks (exam dates) where *pacing*, not duration, is the real ambiguity,
from (c) vague-target tasks ("Learn Python") where skill-level target
is the real ambiguity.
- A proposed new search-grounded step (same architectural pattern as the
  existing `search_subagent.py` / `location_subagent.py`) to research
  realistic durations for novel tasks not covered by curated examples --
  cross-reference multiple sources, disregard course-marketing language
  ("master X in 7 days"), report a range with a stated reason, never a
  fabricated precise number.
- Source material for both the built and unbuilt halves of item 3:
  `DoneHo_Seed_Reference_v3.md` -- 8 general rules (including the
  Combinability Tier terminology, kept explicitly distinct from DoneHo's
  existing personalized `FocusLevel`), 100 tasks across all 10 real
  `GoalCategory` values, and 36 cross-persona patterns (gig workers,
  elderly-parent caregivers, students, night-shift/rotating-shift
...
- **Correction needs to be persistent, not one-time.** If a user corrects
  ...
  that same task respect it -- not just the one instance the user happened
  to fix. Generalizes beyond any single example: whenever the AI infers a
  starting point, level, or scope within a task, the user needs a fast,
  specific way to correct it, and that correction must stick going
  forward.
...
- **Near-term, low-effort:** Smart Spend already generates real,
  ...
  should not be conflated with it when this gets pitched to anyone.
